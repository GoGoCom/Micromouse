...
void OldMouse::update_sensors(void) {
...
OldMouse::OldMouse( Simulation *simp )
{
...
bool  OldMouse::WalkToGoal ( ){
	return FindGoal( false );
}

bool  OldMouse::RunToGoal  ( ){
	return FindGoal( true );
}

bool OldMouse::FindGoal ( bool mode ) {
	...
	readyToGo();
	  ...
	  if( mode == false ) { // false - from searching, true - optimized path
		...
		printf("log Old Adult mouse goes to Goal position\n");
...
bool OldMouse::StartToGoal( uint8_t startDir) {
...
bool OldMouse::GoalToStart( uint8_t startDir) {
...
int  OldMouse::HowToGo (  uint8_t * mx, uint8_t * my, uint8_t * mdir, bool direction ) {
...
#if SIMULATION == 1
	// measure the distance to frontWall
	int cnt = length_frontwall()/2 ;
#else
	int cnt = 0;
#endif

     if(cnt > 2 ) printf("log away %d\n", cnt);
       ...
       if( found_start|| found_goal ) {
        ...
        if( !this_node->visited ) { // No visited
			...
			*my = this_node->y;
			...
			Buildshortcut( mouse_queue, mx, my, this_node->x, this_node->y, cur_dir, false);
...
uint8_t OldMouse::MoveAction(Queue * this_queue, uint8_t startDir ) {
	Queue *temp_queue;
...
int OldMouse::Buildshortcut( Queue * this_queue, uint8_t *px, uint8_t *py, uint8_t lx, uint8_t ly, uint8_t dir, bool state) {
